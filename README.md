...
![image](https://raw.githubusercontent.com/davidbintang/Jarkom_Modul5_Lapres_B06/master/gambar%201.png)
...
# Router
xterm -T SURABAYA -e linux ubd0=SURABAYA,jarkom umid=SURABAYA eth0=tuntap,,,10.151.74.29 eth1=daemon,,,switch3 eth2=daemon,,,switch2 mem=96M &
...
Jawab : <br>
menggunakan metode VLSM, dan berikut adalah gambar pembagian subnet, pohon IP dan table IP nya.

- Pembagian Subnet

![VLSM](https://raw.githubusercontent.com/davidbintang/Jarkom_Modul5_Lapres_B06/master/gambar%202.png)


- Pohon IP

![Praktikum5](https://raw.githubusercontent.com/davidbintang/Jarkom_Modul5_Lapres_B06/master/gambar%203.jpg)


- Table IP

![Pembagian IP](https://raw.githubusercontent.com/davidbintang/Jarkom_Modul5_Lapres_B06/master/gambar%204.png)
...
```
Malang	    10.151.83.58
Mojokerto   10.151.83.59
Batu	     192.168.2.2
Sidoarjo    192.168.0.2
Surabaya    10.151.74.30
Kediri	    192.168.2.6
Gresik	    192.168.1.2
Madiun	    192.168.2.10
...
subnet 192.168.0.0 netmask 255.255.255.0 {
    ...
    option domain-name-servers 10.151.83.58;
...
subnet 192.168.1.0 netmask 255.255.255.0 {
    ...
    option domain-name-servers 10.151.83.58, 202.46.129.2;
    default-lease-time 600;
    max-lease-time 7200;
}

subnet 10.151.83.56 netmask 255.255.255.248 {
...
```
iptables -t nat -A POSTROUTING -o eth0 -j SNAT --to 10.151.74.30
```

Penjelasan : <br>
dengan menggunakan table nat chain POSTROUTING melalui eth0, setiap paket yang menuju keluar akan dirubah sourcenya menjadi 10.151.74.30<br>
...
Pada UML Surabaya, ketikkan perintah seperti dibawah
```
iptables -A FORWARD -p tcp -i eth0 --dport 22 -d 10.151.83.56/29 -j DROP
```

Penjelasan : <br>
pada UML SURABAYA dibuat iptables dengan chain FORWARD, setiap paket dengan protokol tcp, incoming interface eth0 9dari luar), dan port tujuan 22 serta subnet tujuan 10.151.83.57/29 akan didrop

- **Testing**
1. Lakukan ```apt-get update```
2. Install netcat pada seluruh UML dengan perintah ```apt-get install netcat```
3. pada UML SURABAYA, ketikkan ```nc 10.151.83.58 22```
...
- **Testing**
Lakukan ping ke IP MALANG (10.151.83.58) atau IP MOJOKERTO (10.151.83.59) di 4 UML (Misalkan UML BATU, KEDIRI, MADIUN dan PROBOLINGGO)
...
3. Lakukan ```ping 10.151.83.58``` pada UML GRESIK dan SIDOARJO
...
1. Lakukan ```apt-get update```
2. Install netcat pada seluruh UML dengan perintah ```apt-get install netcat```
3. pada Putty Yudhistira, ketikkan ```nc 10.151.83.58 22```
